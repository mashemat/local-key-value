...
int setup_buffers(struct ctrl_blk *cb)
{
	...
	if (cb->is_client) {
	   ...
	   buckets = memalign(4096,   sizeof(struct hopscotch_bucket) * (1 << HOPSCOTCH_INIT_BSIZE_EXPONENT) ); 
...
//sleep(2);
        ...
        server_hash_area = hhead->buckets;
		//memset((int*) server_hash_area, 0 ,  SERVER_AREA   ); // REQ_AC * FARM_INLINE_READ_SIZE					
		server_hash_area_mr = ibv_reg_mr(cb->pd, 
			(struct hopscotch_bucket *) server_hash_area, sizeof(struct hopscotch_bucket) * 1<<hhead->exponent, FLAGS);				
...
exponent =  HOPSCOTCH_INIT_BSIZE_EXPONENT;
    ...
    memset(buckets, -1, sizeof(struct hopscotch_bucket) * (1 << exponent));
    if ( NULL == ht ) {
